...
REM Instaluj pakiety POJEDYNCZO (żeby zobaczyć który powoduje problem)
echo Instalowanie pakietów pojedynczo...
echo.

echo   - opencv-python...
%PYTHON_CMD% -m pip install opencv-python --no-warn-script-location >> "%BUILD_LOG%" 2>&1
echo   - onnxruntime-directml...
%PYTHON_CMD% -m pip install onnxruntime-directml --no-warn-script-location >> "%BUILD_LOG%" 2>&1
echo   - PyQt5...
%PYTHON_CMD% -m pip install PyQt5 --no-warn-script-location >> "%BUILD_LOG%" 2>&1
echo   - pywin32...
%PYTHON_CMD% -m pip install pywin32 --no-warn-script-location >> "%BUILD_LOG%" 2>&1
echo   - mss...
%PYTHON_CMD% -m pip install mss --no-warn-script-location >> "%BUILD_LOG%" 2>&1
echo   - pillow...
%PYTHON_CMD% -m pip install pillow --no-warn-script-location >> "%BUILD_LOG%" 2>&1
echo   - keyboard...
%PYTHON_CMD% -m pip install keyboard --no-warn-script-location >> "%BUILD_LOG%" 2>&1
echo   - psutil...
%PYTHON_CMD% -m pip install psutil --no-warn-script-location >> "%BUILD_LOG%" 2>&1
echo   - pyyaml...
%PYTHON_CMD% -m pip install pyyaml --no-warn-script-location >> "%BUILD_LOG%" 2>&1
echo   - pyinstaller...
%PYTHON_CMD% -m pip install pyinstaller --no-warn-script-location >> "%BUILD_LOG%" 2>&1
